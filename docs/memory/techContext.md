...
- **Package manager**: Yarn `1.22.22` (classic, enforced via `packageManager` field)
- **Browser targets (production)**: >0.2% market share, no IE≤11, no Safari<12, no Chrome<70, no Edge<79
...
> Core canvas/element state is plain React class component state (`AppState`) — not Jotai.
...
- **Docker**: multi-stage build — Node builds static assets → nginx serves them (`Dockerfile`)
- **Vercel**: `vercel.json` points output at `excalidraw-app/build`, sets CORS/security headers
