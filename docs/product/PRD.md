...
Excalidraw — це **безкоштовний відкритий whiteboard-інструмент**, який дозволяє швидко створювати діаграми, схеми та нотатки з характерним **hand-drawn стилем**. Офіційний опис із PWA-маніфесту: *"Excalidraw is a whiteboard tool that lets you easily sketch diagrams that have a hand-drawn feel to them"* (`excalidraw-app/vite.config.mts`, рядок 217).
...
16 типів інструментів (`ToolType` у `packages/excalidraw/types.ts`, рядки 143–159):
...
- Логічні контейнери для групування елементів (аналог artboard у Figma).
- **Magic Frame** — контейнер для AI-генерації (Text-to-Diagram, beta).
...
- **56 файлів локалей** у `packages/excalidraw/locales/`.
...
3. **Немає власного AI-бекенду** — Text-to-Diagram (beta) і Diagram-to-Code працюють лише з зовнішнім `VITE_APP_AI_BACKEND`; серверна частина у репо відсутня.
...
| Monorepo | Yarn workspaces, 6 пакетів у `packages/` |
| Tests | Vitest |
