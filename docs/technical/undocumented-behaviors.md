...
- **Що відбувається**: `Store.scheduleCapture()` викликається з багатьох місць у кодовій базі. Метод планує обчислення дельти для undo stack. Множинні точки виклику створюють implicit state machine, де порядок `schedule → commit` є критичним для коректної роботи undo/redo.
...
- **Файл**: `packages/element/src/frame.ts:752`
- **Що відбувається**: Функція `isElementInFrame()` перевіряє чи елемент знаходиться у frame, але робить це з O(n) складністю для кожного виклику. При виклику для всіх елементів сцени це стає O(n²) bottleneck для великих сцен.
- **Де задокументовано**: тільки коментар `// TODO: this a huge bottleneck for large scenes, optimise`
- **Ризик**: AI може спробувати додати кешування, але frame membership змінюється при drag — наївний кеш буде stale і спричинить візуальні артефакти з елементами, які "застрягли" в неправильному frame.
...
- **Файл**: `packages/element/src/delta.ts:726`
- **Що відбувається**: History delta filtering припускає, що previous appState не містить references до deleted elements. Але це не завжди правда (наприклад, при remote updates). Через це можуть виникати "порожні" undo/redo кроки, де користувач натискає Ctrl+Z і нічого не відбувається.
