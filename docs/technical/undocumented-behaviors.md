...
- **Серйозність**: `bug`
- **Що відбувається**: Функція `isElementInFrame()` має складність O(n) на кожний виклик, тому що внутрішньо викликає `getElementsInGroup()` (ітерує всі елементи для пошуку по groupId) та `getSelectedElements()` (також ітерує всі елементи). Коли ця функція викликається для всіх елементів сцени (наприклад у `frame.ts:668` через `elementsToFilter.forEach`), загальна складність стає O(n²). На великих сценах з сотнями елементів це спричиняє помітну performance деградацію.
