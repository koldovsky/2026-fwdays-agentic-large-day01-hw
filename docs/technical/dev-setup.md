...
## Summary
- Practical onboarding guide from clone to local validation and first PR.
- Covers prerequisites, environment setup, run/build commands, and common troubleshooting.
...
## Actions
- Keep command examples aligned with root `package.json` scripts and CI workflows.
- Refresh environment guidance when `vite.config.mts` or root env conventions change.
